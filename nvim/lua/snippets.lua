...
local function jdocsnip(args, _, old_state)
    ...
    if vim.tbl_count(args[3]) ~= 1 then
        ...
        if old_state and old_state.ex then
            ins = i(insert, old_state.exget_text())
        else
            ins = i(insert)
        end
        vim.list_extend(
...
ls.add_snippets("tex", {
    ...
    \frac{<>}{<>}
     ...
     s("tab", fmt([[
         ...
         { i(1, "caption"), i(2, "label"), i(3, "lcc"), i(0, "tab") },
         { delimiters = "<>" }
     )
     ),
...
-- see DOC.md/LUA SNIPPETS LOADER for some details. require("luasnip.loaders.from_lua").load({ include = { "c" } })
